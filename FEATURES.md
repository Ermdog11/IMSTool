...
| **Web search (Brave)** | A real open-web search alongside the ~70 curated RSS/news queries — catches stories no standing query anticipated. Paste a Brave Search API key into Settings to connect (encrypted at rest); runs only on the 3x/day rolling-digest cron, not the manual/30-min scan, to keep the metered cost small (~$1-2/month). Live and connected. |
