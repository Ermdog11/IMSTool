...
| **X/Twitter search (fast breaking-news signal)** | A viral or headline-worthy social moment (a rival player's comments, a recruiting bombshell) often breaks on X before any RSS feed or news site covers it. Its own dedicated ~30-minute cron searches for high-engagement posts mentioning the beat, feeds them through the same rating pipeline as everything else, and auto-drafts + emails + posts to Team Chat on anything rating 4+ (desktop push at 5+) — same breaking-news treatment RSS/Brave stories get, just on a faster clock built specifically for this source. Paste an X API Bearer Token into Settings to connect (encrypted at rest, metered pay-per-use billing on X's side). Built and wired in; not yet verified against a live token. |
